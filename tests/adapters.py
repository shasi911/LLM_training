...
from model import (
    AdamW, Embedding, Linear, MultiHeadSelfAttention, MultiHeadSelfAttentionWithRoPE,
    RMSNorm, RoPE, SwiGLU, TransformerBlock, TransformerLM,
...
def run_multihead_self_attention_with_rope(
    ...
    mha.load_state_dict(
        {"q_proj.W": q_proj_weight, "k_proj.W": k_proj_weight,
         "v_proj.W": v_proj_weight, "o_proj.W": o_proj_weight},
        strict=False,  # RoPE cos/sin buffers are pre-computed, not loaded
    )
...
def run_transformer_block(
    ...
    # Checkpoint uses .weight for all layers and output_proj for o_proj;
    # our Linear modules use .W and MHA uses o_proj -- remap before loading.
    remapped = {}
    for k, v in weights.items():
        new_k = k.replace("attn.output_proj", "attn.o_proj")
        if new_k.endswith(".weight") and not new_k.startswith("ln"):
            new_k = new_k[: -len(".weight")] + ".W"
        remapped[new_k] = v

    # strict=False: RoPE cos/sin buffers are pre-computed, not in the checkpoint
...
def run_transformer_lm(
    ...
    # Remap checkpoint keys to our module's key names:
    #   token_embeddings.weight  -> token_embeddings.embedding
    #   lm_head.weight           -> lm_head.W
    #   attn.output_proj         -> attn.o_proj
    #   *.weight (non-RMSNorm)   -> *.W
    remapped = {}
    for k, v in weights.items():
        new_k = k.replace("attn.output_proj", "attn.o_proj")
        new_k = new_k.replace("token_embeddings.weight", "token_embeddings.embedding")
        if new_k.endswith(".weight") and not (
            new_k == "token_embeddings.embedding"
            or new_k.endswith("ln1.weight")
            or new_k.endswith("ln2.weight")
            or new_k.endswith("ln_final.weight")
        ):
            new_k = new_k[: -len(".weight")] + ".W"
        remapped[new_k] = v

    # strict=False: RoPE cos/sin buffers are pre-computed, not in the checkpoint
    lm.load_state_dict(remapped, strict=False)
